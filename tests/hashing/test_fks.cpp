...
namespace test_fks {
    ...
    // Use size_t for A, C, M, Offset to avoid truncation (e.g., 1u<<16 into uint16_t).
    ...
    constexpr auto make_table_from_seq(std::integer_sequence<K, Ks...>) {
        return etools::hashing::fks_hash_gen<K>::template generate<Ks...>();
    }
...
TEST(FKS_Small, TrivialSets) {
    {
        using G = fks_hash_gen<std::uint8_t>;
        constexpr auto T = G::generate<42u>();
        static_assert(T.size() == 1, "N");
        EXPECT_EQ(T(42u), 0);
        EXPECT_EQ(T(41u), T.not_found());
    }
    {
        using G = fks_hash_gen<std::uint16_t>;
        constexpr auto T = G::generate<1u,2u,3u,4u,5u>();
...
TEST(FKS_Medium, DenseSequential_1024_uint16) {
    ...
    // Sequence 0..1023
    constexpr auto T = test_fks::make_table_from_seq(
...
TEST(FKS_Medium, AffinePermutation_2048_uint16) {
    ...
    constexpr auto T = test_fks::make_table_from_seq(Perm{});
